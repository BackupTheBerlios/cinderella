# $Id: IcmpPolicy.rb,v 1.1 2003/06/05 18:05:37 ak1 Exp $

class IcmpPolicy

  def initialize(src_re,dst_re,pol_list)
    @src_re = src_re
    @dst_re = dst_re
    @codes = Array.new
  ...
  def match_policy(pkt)
    ...
    nil!=@src_re.match(src_str) and nil!=@dst_re.match(dst_str) and code>=0 and code<=18 and @codes[code]
  end
